...
export class RoomConfigComponent implements OnInit, OnDestroy {
  ...
  resolrutionProperties : string[];
	...
	initNicknameAndSubscribeToChanges() {
		...
		const nickname = this.storageSrv.get(Storage.USER_NICKNAME) || '99_변경필요_' + Math.floor(Math.random() * 100);
	...
	joinSession() {
      ...
      //mic off default
      // ecept 00_
      if (!(testnick.includes('00_') || testnick.includes('2층_민원') || testnick.includes('9층_회의실') || testnick.includes('수석부원장')) ){
